...
# ------------------------------------------------------------------
# Task 4: TUI Monitor -- removed in v0.4 (replaced by Phos studio)
# These tests were for the Rich TUI which has been deleted.
# ------------------------------------------------------------------

# tui.py is now a removed stub; TUIMonitor must not be importable
def test_tui_module_removed():
    """Verify TUIMonitor is not importable in v0.4 (replaced by Phos studio)."""
    import importlib
    import sys
    # Remove any cached module entry so the import is fresh
    sys.modules.pop("mud_puppy.tui", None)
    with pytest.raises(ImportError):
        importlib.import_module("mud_puppy.tui")
