...
class TestInt4PackUnpack:
    ...
    @pytest.mark.parametrize("out_f,in_f", [(0, 0), (4, 0)])
    def test_pack_unpack_empty_tensor_roundtrip(self, out_f, in_f):
        """A2: pack_int4 -> unpack_int4 on zero-dimension tensors must not crash.

        (0, 0) -- fully empty weight matrix.
        (4, 0) -- weight with rows but no columns (e.g. a layer with
                  in_features=0, valid in some architectures or after pruning).
        """
        w = torch.empty(out_f, in_f, dtype=torch.int8)
        packed = pack_int4(w)
        assert packed.numel() == 0, (
            f"pack_int4({out_f},{in_f}) must return empty tensor, got numel={packed.numel()}"
        )
        unpacked = unpack_int4(packed, out_f, in_f)
        assert unpacked.shape == (out_f, in_f), (
            f"unpack_int4({out_f},{in_f}) returned wrong shape: {unpacked.shape}"
        )
